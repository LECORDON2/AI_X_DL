...
# 있던 데이터 프레임에서 이후 예측을 위해서 필요 없는 columns를 없애고 Training데이터와 Target를 나눕니다.
...
########################################################################### 최종 정확도 계산 파트 그냥 타켓 정수로 바꾼것 84% 0과 1이 너무 많음
...
# 위의 방식으로 할 때는 소수점 아래 숫자를 무시하기 때문에 0과 1인 데이터가 너무 많이 나옴에 따라
# 다른 방식으로 하기 위해 Global_Sales의 값에 100을 곱하고 정수로 바꾼다.
VG_Target_100 = VG_Target.mul(100)
VG_Target_100_int = VG_Target_100.astype({'Global_Sales': 'int'})

############################################################################# 타겟에 100 곱한것을 일단 돌려보면 5%로 나오는 것을 볼 수 있습니다.
# values 종류가 너무 많아 정확도가 떨어지는 것으로 보입니다.
...
############################################################################# 타겟에 100 곱한것을 25로 나누어 다시 분류한 것을 다시 예측을 해봅니다.
# 53% 정도로 나오게 됩니다.
...
# 전체 Target 안 values에서 0과 1같이 판매량이 저조한 비디오게임이 너무 많은 비율을 차지하고 있어서 이것을 제거하면 혹시 더 정확히 예측이 될지 알아보기 위해
...
########################################################################### Global_Sales에 0과 1일 포함된 행을 제거한 후 다시 계산하니 23%가 나옵니다.
...
############################################################################# 최종 정확도 계산 파트 타겟에 100 곱한것을 일단 돌려보면 5%
# values 종류가 너무 많아 정확도가 떨어진다.
...
############################################################################# 최종 정확도 계산 파트 타겟에 100 곱한것을 25로 나누어 다시 분류한다.
# 53% 정도로 나오게 된다.
...
########################################################################### Global_Sales에 0과 1일 포함된 행을 제거한 후 다시 계산하니 25%
...
# VG_Training_int['Global_Sales'] = VG_Target_25_int
# VG_Training_int.astype({'Global_Sales': 'int'})
# print(VG_Training_int) ?????????????????

# print(len(VG_Target_100_int_list_25))
# print(VG_Target_100_int_list_25)
# print(VG_Target_100_int.value_counts())
# print(VG_Target_int.value_counts())

# print(VG_Target)
# VG_Target_int_list = VG_Target_int.tolist()
# print(VG_Target_int)
# print(VG_Target_int_list)
# print(VG_Target_int_list.count(0))

# target값을 정수로? 경계를 구분해서 정수로 바꿔야 정확도
# print(VG_Target)



# VG_Head_num = VG_Training.head(10)
...
# print(len(VG_platform_my_list_num))
# print(VG_platform_my_list_num)
# VG_Head = VG_Training.head(10)

# VG_Publisher_set = set(VG_Publisher_list) 
# VG_Publisher_my_list = list(VG_Publisher_set)




# list_STR = ['감자', '국수', '메밀', '감자', '수박', '감자', '수박']
# list_KIND = ['감자', '국수', '메밀', '수박']


# list_fi = []
# for i in list_STR:
#     if i in list_STR:
#         list_fi.append(list_KIND.index(i))
...
# for i in range(len(VG_platform_my_list)):
#     VG_Head_Fi = VG_Head.replace({'Platform' : VG_platform_my_list[i]}, i)

# df = VG_Head.replace({'Platform' : VG_platform_my_list[i]}, i)




# print(VG_platform_my_list)

# VG_Sh = VG_Training.sample(frac=1)



# print(len(VG_Publisher_my_list))
# print(VG_Publisher_my_list.index('Nintendo'))
